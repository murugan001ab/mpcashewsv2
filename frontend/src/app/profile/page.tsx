...
function ProfileContent() {
  ...
  const initialTab: TabId = TAB_IDS.includes(tabParam ?? "") ? (tabParam as TabId) : "orders";

  const [active, setActive] = useState<TabId>(initialTab);
  const { user } = useContext(AuthContext);

  // Keep the active tab in sync if the query param changes (e.g. the user
  // clicks "Saved Addresses" in the navbar while already on /profile).
  useEffect(() => {
    if (tabParam && TAB_IDS.includes(tabParam) && tabParam !== active) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 lg:px-8 py-10 md:py-16 pt-28 md:pt-32 min-h-screen">
      {/* Header Card */}
