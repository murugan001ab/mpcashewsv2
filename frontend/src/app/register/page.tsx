...
// Ported from pages/Login/Register.tsx.
...
import type { RegisterFormState, AlertState, PasswordRule } from "@/types/authUi";
...
export default function SignupPage() {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setAlert({ type: "error", msg: getErrorMessage(err, "Registration failed. Try again.") });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthLayout>
      ...
      <p className="text-stone-500 text-sm mt-1 mb-7">Join thousands of happy customers</p>
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    ...
                    style={{ background: i <= strength ? STRENGTH_COLORS[strength] : "#e7e5e4" }}
                  />
                ))}
              </div>
              <div className="text-[11px] font-semibold" style={{ color: STRENGTH_COLORS[strength] ?? "#a8a29e" }}>
                  ...
                  return (
                    <span
                      ...
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium border transition-all ${
                        pass ? "bg-green-50 border-green-200 text-green-700" : "bg-stone-50 border-stone-200 text-stone-500"
                      }`}
                    >
                      {pass ? "✓" : "✗"} {r.label}
                    </span>
