...
export default function ProductDetails() {
  ...
  // New state to track the chosen variant
  ...
  useEffect(() => {
    api.get(`products/${id}`)
      .then(res => {
        ...
        // If the product has variants, set the smallest weight as default
        if (data.variants && data.variants.length > 0) {
          const sortedVariants = [...data.variants].sort((a, b) => a.weight_grams - b.weight_grams);
  ...
  // ── Derived State (Based on Selected Variant) ────────────────────────────
  const variants = product?.variants ? [...product.variants].sort((a, b) => a.weight_grams - b.weight_grams) : [];
  const activeVariant = variants.find(v => v.id === selectedVariantId) || variants[0] || {};

  const originalPrice = parseFloat(activeVariant.price || product?.price || 0);
  const finalPrice    = parseFloat(activeVariant.discounted_price || activeVariant.price || product?.discounted_price || product?.price || 0);
  ...
  // Reset quantity if the selected variant stock is lower than current quantity
  useEffect(() => {
  ...
  const handleAddToCart = async () => {
    ...
    setAddingCart(true);
    try {
      // Include variant_id in the payload
  ...
  const handleBuyNow = () => {
    // Pass variant_id through URL params for the payment/checkout page
    navigate(`/payment?buynow=1&product_id=${product.id}&variant_id=${activeVariant.id}&qty=${quantity}`);
  };
  ...
  if (!product) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 pt-24">
        ...
        <p className="text-brand-brown/60 mb-8">This product might have been removed or is currently unavailable.</p>
        <button 
          onClick={() => navigate("/")}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16 pt-24 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 mb-16">
        ...
        <div className="lg:col-span-6 flex flex-col gap-4">
          <div className="relative bg-brand-cream/30 border border-brand-brown/5 rounded-3xl aspect-square flex items-center justify-center overflow-hidden shadow-sm group">
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
              <button
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm backdrop-blur-sm border
                  ${wishlisted ? "bg-brand-orange border-brand-orange text-white scale-110" : "bg-white/90 border-white text-brand-brown/50 hover:text-brand-orange"}`}
              ...
              {thumbImgs.map((src, i) => (
                <button
                  ...
                  className={`w-20 h-20 flex-shrink-0 rounded-2xl overflow-hidden border-2 transition-all duration-200 bg-brand-cream/30 ${
                    selectedImg === i ? "border-brand-orange ring-2 ring-brand-orange/20" : "border-transparent opacity-70 hover:opacity-100"
                  }`}
        ...
        <div className="lg:col-span-6 flex flex-col pt-2 lg:pt-6">
          ...
          <div className="flex flex-wrap items-end gap-3 mb-6">
            ...
            {hasDiscount && (
              <span className="text-lg font-bold text-brand-brown/40 line-through decoration-brand-brown/30 mb-1">
          ...
          <div className="flex items-center gap-2 mb-6">
            <span className="relative flex h-2.5 w-2.5">
              {inStock && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-green opacity-40"></span>}
              <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${inStock ? "bg-brand-green" : "bg-red-500"}`}></span>
            </span>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8">
            <div className="flex items-center justify-between w-full sm:w-36 h-14 bg-white border border-brand-brown/10 rounded-xl px-2 shadow-sm">
              ...
              <span className="font-bold text-lg text-brand-black tabular-nums">
                {quantity}
              </span>
              <button 
                onClick={() => handleQty(1)} 
      ...
      <div className="max-w-4xl mx-auto mt-16 md:mt-24">
        <div className="flex border-b border-brand-brown/10 overflow-x-auto hide-scrollbar mb-8">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === "description" && (
                <div className="prose prose-brand max-w-none text-brand-brown/80 leading-loose">
