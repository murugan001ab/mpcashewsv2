...
import { ShoppingCart, ArrowRight, Trash2, ArrowLeft, Minus, Plus, Package } from "lucide-react";
...
export default function Cart() {
  ...
  // ── Calculations ─────────────────────────────────────────────────────────
  const subtotal = cart.reduce((s,i) => s + (i.price ?? i.price_at_add ?? i.product?.price ?? 0) * i.quantity, 0);
        ...
        <div className="lg:w-2/3 flex flex-col">
          ...
          <AnimatePresence>
            {cart.map(item => {
              ...
              const itemPrice = item.price ?? item.price_at_add ?? p.price ?? 0;
