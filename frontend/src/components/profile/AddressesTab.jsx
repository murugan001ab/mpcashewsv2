...
import { getAddresses, addAddress, updateAddress, deleteAddressById } from "../../services/addressService";
...
export default function AddressesTab() {
  ...
  const saveAddress = async (e) => {
    ...
    try {
      if (form.id) {
        await updateAddress(undefined, form.id, form);
      } else {
        await addAddress(undefined, form);
      }
      setForm(null);
      await load();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteAddressById(undefined, id);
      await load();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="max-w-4xl">
      ...
      {/* ── Address Form ──────────────────────────────────────────────────── */}
      {form ? (
        <div className="bg-gray-50 border border-brand-brown/10 rounded-2xl p-6 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          {addresses.map((addr) => (
            <div
              ...
              className={`group relative bg-white border p-6 rounded-2xl transition-all duration-300 shadow-sm hover:shadow-md ${
                addr.is_default ? "border-brand-orange/40 ring-1 ring-brand-orange/10" : "border-brand-brown/10 hover:border-brand-brown/30"
              }`}
            >
              {addr.is_default && (
