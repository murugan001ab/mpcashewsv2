...
function HomeContent() {
  ...
  const findCartItem = (p: ProductWithVariant) =>
    cartItems.find((i) => i.variant?.id === p.selectedVariant?.id || i.product?.id === p.id);
  ...
  const handleBuyNow = (product: ProductWithVariant) =>
    router.push(`/payment?buynow=1&product_id=${product.id}&qty=1`);
      ...
      <section className="relative min-h-[90vh] flex flex-col justify-end overflow-hidden bg-brand-black">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-8 pb-16 pt-24 flex flex-col gap-8">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              onClick={() => router.push("/becomepatner")}
            ...
            {filteredProducts.map((p) => (
              <motion.div key={p.id} variants={cardAnim}>
                <ProductCard
                  ...
                  buttonTitle={cartItems.find((i) => i.product?.id === p.id) ? "Update Cart" : "Add to Cart"}
                />
              </motion.div>
            ))}
          </motion.div>
